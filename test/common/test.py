...
def accessOracle(query):
    ...
    c = None
    try:
        conn = oracledb.connect(user=r'owner_31_bpi_3_0', password='owner31bpi', dsn=dsn_tns)  # if needed, place an 'r' before any parameter in order to address special characters such as '\'. For example, if your user name contains '\', you'll need to place 'r' before the user name: user=r'User Name'
        c = conn.cursor()
        c.execute(query)  # use triple quotes if you want to spread your query across multiple lines
        result = c.fetchall()
    except Exception as e:
        print(f"Error occurred: {e}")
        result = None  # 或者可以根据需要抛出异常    
    finally:
        if c:
            c.close()
        if conn:
...
def packageinfo(lpc):
    ...
    # UrgencyPackageQuery = "select lpc from  WC_PACKAGEINFO where lpc = {} ".format(lpc)
    baginformation = accessOracle(UrgencyPackageQuery)
