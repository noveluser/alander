    ...
    try:
        ...
        with open(input_filepath, 'r', encoding='utf-8') as infile:
            with open(output_filepath, write_mode, encoding='utf-8') as outfile:
                for line in infile:
                    xml_fragment = line
                    # xml_fragment = xml_fragment.replace("&amp;", "") # 第一步：去除 &amp; 字符串
                    if perform_entity_replace:
                        xml_fragment = xml_fragment.replace("&lt;", "<").replace("&gt;", ">")
                        # xml_fragment = correct_xml_string_formatted(xml_fragment)
...
if __name__ == "__main__":
    oldfilename = "1"
    input_xml_file = "d://1//3//{}.txt".format(oldfilename)
    output_xml_file = "d://1//3//processed_{}.xml".format(oldfilename)
