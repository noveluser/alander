...
def collectbaginfo():  # 先处理有LPC的
    ...
    for row in data:
        ...
        # else:
        #     lpc = "NULL"
        searchlpcquery = "select pid from temp_arrivebag where register_time > curdate() and pid = {}".format(row[1])
        pid = cursor.run_query(searchlpcquery)
        if not pid:
            register_time = row[3].strftime("%Y-%m-%d %H:%M:%S")
            deregister_time = row[5].strftime("%Y-%m-%d %H:%M:%S")
            orignal_sqlquery = "insert into ics.temp_arrivebag (register_time, lpc, pid, flightnr, REGISTER_LOCATION, deregister_time,DEREGISTER_LOCATION,duringtime)  values ('{}',{},{},'{}','{}','{}','{}', {})".format(register_time, row[0], row[1], row[2], row[4], deregister_time, row[6], during_time)
...
def writerinfluxdb():
    # 写入influxdb
    searchbag = "select * from temp_arrivebag where register_time > curdate() "
    # searchbag = "select * from temp_arrivebag where TO_DAYS(NOW())-TO_DAYS(register_time) < 1"   # 临时写两天数据
...
def main():
    ...
    schedule.every(3600).seconds.do(writerinfluxdb)
    while True:
        schedule.run_pending()
        # s = sched.scheduler(time.time, time.sleep)
        # s.enter(60, 2, collectbaginfo, (startID, endID))
        # s.enter(3600, 1, writerinfluxdb)
        # s.run()
