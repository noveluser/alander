...
def main():
    ...
    LT_index = ['TIME', 'CI值机行李量', "diff_CI值机行李量", '分拣机导入口', "diff_分拣机导入口", "DC处理量", "diff_DC处理量", "分拣机卫星厅导出口", "diff_分拣机卫星厅导出口", "VCC", "diff_VCC", "SAT到港行李量", "diff_SAT到港行李量", "T3到港行李量", "diff_T3到港行李量", "T3到港分拣环导入口", "diff_T3到港分拣环导入口", "SAT离港分拣环导入口", "diff_SAT离港分拣环导入口", "隧道离港", "diff_隧道离港", "隧道到港", "diff_隧道到港"]
...
def onedaydf(xlsfile):
    ...
    df['VCC'] = df["VCC04"] + df["VCC05"] + df["VCC06"] + df["VCC07"] + df["VCC08"] + df["VCC20"] + df["VCC21"] + df["VCC22"] + df["VCC23"] + df["VCC24"] + df["VCC38"] + df["VCC39"]
    ...
    df["隧道离港"] = df["D01_Tun"] + df["D02_Tun"]
    df["隧道到港"] = df["A01_Tun"] + df["A02_Tun"]
    origin_index = ["CI值机行李量", "分拣机导入口", "分拣机导入口", "DC处理量", "分拣机卫星厅导出口", "分拣机卫星厅导出口", "VCC", "SAT到港行李量", "T3到港行李量", "T3到港分拣环导入口", "SAT离港分拣环导入口", "隧道离港", "隧道到港"]
