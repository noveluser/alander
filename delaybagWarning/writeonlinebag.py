...
def collectbaginfo():
    ...
    c = conn.cursor()
    sqlquery = "SELECT  EVENTTS,  lpc,  bid,  pid,  DEPAIRLINE,  DEPFLIGHT,  CURRENTSTATIONID, L_DESTINATIONSTATIONID , idevent FROM  WC_PACKAGEINFO  WHERE  EVENTTS >= to_date( to_char( SYSDATE - 1 / ( 24 * 60 ), 'yyyy-mm-dd hh24:mi:ss' ), 'yyyy-mm-dd hh24:mi:ss' )  AND TARGETPROCESSID LIKE 'BSIS%'   AND EXECUTEDTASK = 'Registration'   AND DEPAIRLINE IS NOT NULL  ORDER BY  EVENTTS"
    ...
    for row in c:
        ...
        create_time = localTime.strftime("%d-%m-%Y %H:%M:%S")
        sqlquery = "insert into ics.onlinebag (created_time, lpc, bid, pid, DEPAIRLINE, DEPFLIGHT, currentstation,destination)  values ('{}',{},{},{},'{}',{},{},'{}')".format(create_time, row[1], row[2], row[3], row[4], row[5], row[6], row[7])
