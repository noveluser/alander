...
logging.basicConfig(
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename='/data/package/crontab/log/secondscanbags_status.log',
                    filemode='a')
...
def updatebagstatus(process_id):
    ...
    findbagquery = f"""
            ...
            FROM (
                ...
                WHERE 
                    status IS NULL 
                    AND bsm_time >= DATE_ADD(NOW(), INTERVAL - 30 MINUTE )
                    AND flighttype = 'D'
                    AND lpc IS NOT NULL
            ) AS subquery
            WHERE rn = 1
            ...
            for item in baginfo:
                ...
                if item[3]:     # 如果有更新记录，则updata，否则无操作
                    ...
                    if int(item[0]) in [41, 42, 81, 82, 220, 221]:    # 已到达弃包处
                        ...
                        status = "unkonwn"
                        logging.info("行李还在输送中，pid={},CURRENTSTATIONID={},DESTINATION={}".format(item[2], item[0], item[5]))
                    ...
                    logging.info("行李位置已更新，行李{},语句{}".format(row[1], optimizal_sqlquery))
                    ...
                    optimizal_sqlquery = updatebagstatus.replace("None", "Null")
                    logging.info(f"行李位置还未更新,{optimizal_sqlquery}")
                    querylist.append(optimizal_sqlquery)
                    # result = cursor.run_query(optimizal_sqlquery)
