...
def updatebagstatus(scanqueuenumber):
    findbagquery = "select lpc, pid, latest_time, status, flightnr, checked from temp_bags where status {} and bsm_time >= DATE_ADD(NOW(),INTERVAL - 3 HOUR) and flighttype = 'D' and lpc is not null".format(scanqueuenumber)
    data = cursor.run_query(findbagquery)
